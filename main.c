...
int main(int argc, const char * argv[]) {
    ...
    while(1){
        ...
        if (recvMsgSize < 0){
            ...
            switch (op) {
                case 0:
                    memcmp(&idTable.oldID , buffer, sizeof(unsigned short));
                    idTable.newID = idTable.oldID + 1;
                    memcmp(response, buffer, recvMsgSize);
                    printf("Send to local DNS\n");
                    /*retransmit the DNS request to the local DNS*/
                    memcmp(response, &idTable.newID, sizeof(unsigned short));
                    ...
                    memcmp(response, buffer, recvMsgSize);
                    memcmp(response, &idTable.oldID, sizeof(unsigned short));
                    rspMsgSize = recvMsgSize;
                    /*send the response to the local client*/
                    
                    if(sendto(sock, buffer, recvMsgSize, 0,(struct sockaddr *) &cliAddr, sizeof(cliAddr))!=recvMsgSize){
                        printf("Send fail!\n");
                        break;
                    }
                    break;
